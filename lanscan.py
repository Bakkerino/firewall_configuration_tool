...
#scannerplugin example, tries to connect to ssh
def ssh(ipaddr):
    s = socket(AF_INET, SOCK_STREAM)
    s.settimeout(0.3)
    if not s.connect_ex((ipaddr,22)):
        s.close()                      
        return 1
    else:
        s.close()
...
def snmp(ipaddr):
    s = socket(AF_INET, SOCK_STREAM)
    s.settimeout(0.3)
    if not s.connect_ex((ipaddr,161)):
        s.close()                      
        return 1
    else:
        s.close()
...
def run():
    ...
    for ip in range(1,254):
        ...
        if ssh(ipaddr):
            print('%s \t ::: \t SSH  \t ::: \t %s' %(ipaddr, getfqdn(ipaddr)))
        if snmp(ipaddr):
            print('%s \t ::: \t SNMP  \t ::: \t %s' %(ipaddr, getfqdn(ipaddr)))
